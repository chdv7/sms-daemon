...
struct hashSmsKey {
    size_t operator()(const smsCacheKey& p) const noexcept {
        return (p.first + (p.first << 6)) ^ (p.first << 16) ^ hash<wstring>{}(p.second);
    }
};
class CRecvSMSPart {
public:
...
struct SmsCacheEntry {
    ...
    size_t nRecvdParts() {
        return nPartsReceived;
    }

    bool isReceived() {
        return nRecvdParts() == nParts();
    }
    time_t lastUpdate{};
...
class ReceivedSMS {
public:
    ReceivedSMS(std::unique_ptr<CRecvSMSPart> onePartSms, const std::string& m_Interface)
