...
CSerial::CSerial() : TSliceEnabled(false) {
  ClearUngetBuf();
...
CSerialType CSerial::GetConnectorType() { return serialTypeNone; }
...
const char* CSerial::GetDeviceName() { return DeviceName.c_str(); }
...
int CSerial::SendChar(int c) { return LastError = SERIAL_ERR_NOTSUPPORTED; }
...
time_t CSerial::GetSysTimeS() { return time(NULL); }
...
bool CSerial::CheckTimeout(bool isSend) {
  ...
  //	TRACE ("CT/O %d %u %u\n", isSend, curTime
  // isSend?m_SendTimeout:m_RecvTimeout );
...
int CSerial::PutBlockEx(const char* buf, unsigned long size,
...
int CSerial::ReceiveChar() { return LastError = SERIAL_ERR_NOTSUPPORTED; }
...
int CSerial::Gets(char* buf, unsigned long size, int separator,
                  unsigned long timeout) {
...
int CSerial::Gets(char* buf, unsigned long size, const char* separators,
                  unsigned long timeout) {
  int chr = 0;
...
int CSerial::GetLastError() { return LastError; }
...
bool CSerial::IsTimeSliceEnabled() { return TSliceEnabled; }
...
int CSerial::SendExpect(const char* send, const char** expect,
                        unsigned long timeout, char* log, long logSize) {
  if (log && (logSize > 0)) {
    *log = '\0';
  ...
  if (errorCode < 0) return LastError = errorCode;
  int rtn = 0;
  ...
  if (expect) {
    ...
    const char** s = expect;
    int nsamples = 0;
    for (; *s != NULL; ++s, ++nsamples)
      ;                                     // counting expect samples
    int* cnttable = new int[nsamples + 1];  // initialize cnttable
    int i = 0;
    for (; i < nsamples; ++i) cnttable[i] = 0;
...
int CSerial::SendExpect(LPBINRECORD send, LPBINRECORD* expect,
                        unsigned long timeout, LPBINRECORD logRec) {
  ...
  if (expect) {
    ...
    for (; *s != NULL; ++s, ++nsamples)
      ;  // counting expect samples
    unsigned long* cnttable =
        new unsigned long[nsamples + 1];  // initialize cnttable
    int i = 0;
    for (; i < nsamples; ++i) cnttable[i] = 0;
...
*/
bool CSerial::IsConnected() { return Connected; }

CSerialHandler::CSerialHandler() { ParentHandler = NULL; }

CSerialHandler::~CSerialHandler() {}

void CSerialHandler::OnConnect(CSerialHandler* handler) {
  if (ParentHandler) ParentHandler->OnConnect(handler ? handler : this);
}

void CSerialHandler::OnDisconnect(CSerialHandler* handler) {
  if (ParentHandler) ParentHandler->OnDisconnect(handler ? handler : this);
}

void CSerialHandler::OnReceiveChar(CSerialHandler* handler) {
  if (ParentHandler) ParentHandler->OnReceiveChar(handler ? handler : this);
}

void CSerialHandler::OnSendChar(CSerialHandler* handler) {
  if (ParentHandler) ParentHandler->OnSendChar(handler ? handler : this);
}

void CSerialHandler::OnIncomingCall(CSerialHandler* handler) {
  if (ParentHandler) ParentHandler->OnIncomingCall(handler ? handler : this);
}
...
CSerialHandler* CSerialHandler::GetParentHandler() { return ParentHandler; }
...
int CSerial::GetFromUnget() {
  if (UngetBufPtr >= UNGET_BUF_SIZE) return LastError = SERIAL_ERR_UNGET_EMPTY;
  return UngetBuf[UngetBufPtr++];
}

int CSerial::Ungetc(int chr) {
  if (!UngetBufPtr) return LastError = SERIAL_ERR_UNGET_OVER;

  UngetBuf[--UngetBufPtr] = chr;
  return SERIAL_ERR_OK;
}

void CSerial::ClearUngetBuf() { UngetBufPtr = UNGET_BUF_SIZE; }
...
int CSerial::UngetBlock(char* buf, unsigned long size) {
  if (size > UngetBufPtr) return SERIAL_ERR_UNGET_OVER;
