...
int CRSSerial::Open(const char* name, int /*timeout*/) {
  ...
  if (name) {
    CString comName;
    if (!strnicmp(name, "com", 3)) comName = "\\\\.\\";
    comName += name;
    ComHandler =
        CreateFile(comName, GENERIC_READ | GENERIC_WRITE, 0, NULL,
                   /*CREATE_ALWAYS*/ OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, 0);
...
int CRSSerial::AdjustRSParams() {
  if (!ComHandler) return LastError = SERIAL_ERR_NOTOPENED;
  DCB dcb;
  memset(&dcb, 0, sizeof(dcb));
  dcb.DCBlength = sizeof(dcb);
  int err = GetCommState(ComHandler, &dcb);
  if (!err) return LastError = SERIAL_ERR_SETPARAMS;
  dcb.fBinary = 1;
...
int CRSSerial::ReceiveChar() {
  if (CheckModemReadyMask()) return LastError;
  ...
  if (IsEchoCancellation &&
      SentEchoSample.size() >
          EchoIndex) {  // If echo cancellation and sample is valid
    if (SentEchoSample[EchoIndex] == x) {          // Echo treceived
      if (++EchoIndex == SentEchoSample.size()) {  // Is last echo character
                                                   // ? -> Reset echo buffer
        SentEchoSample.clear();
        EchoIndex = 0;
      }
      ...
      Ungetc(x);
      for (int i = EchoIndex - 1; i >= 0; i--) Ungetc(SentEchoSample[i]);
      x = GetFromUnget();
      // Reset echo sample
      SentEchoSample.clear();
      EchoIndex = 0;
...
int CRSSerial::PutBlock(const char* buf, size_t size, unsigned long timeout) {
  ...
  if (buf) {
    ...
    while ((size - BytesSent) && !errorCode) {
      ...
      BOOL r =
          WriteFile(ComHandler, writePtr, size - BytesSent, &writenBytes, NULL);
      BytesSent += writenBytes;

      if (!r || (writenBytes == 0L)) {  // No bytes sent or error
        if (::GetLastError()) return LastError = SERIAL_ERR_WERROR;
        if (CheckTimeout(TRUE)) return LastError = SERIAL_ERR_TIMEOUT;
      } else {                     // at least part of buffer has been sent
        if (IsEchoCancellation) {  // If echo cancellation add sample
          for (; writenBytes; --writenBytes) {
            SentEchoSample.push_back(
                *(writePtr++));  // save sample for echo cancellation
          }
        }
      }
    }
...
bool CRSSerial::IsValidAddress(const char* address) {
  ...
  buf[3] = '\0';
  for (int i = 0; i < 3; ++i) buf[i] = toupper(buf[i]);
...
int CRSSerial::PutBlock(const char* buf, size_t size, unsigned long timeout) {
  if (!buf || !size) return SERIAL_ERR_OK;

  if (ComHandler <= 0) return LastError = SERIAL_ERR_NOTOPENED;
  ...
  for (;;) {
    ...
    if (FD_ISSET(ComHandler, &fds)) {
      ...
      if (writen >= 0) {  // Part is writen
        size -= writen;
        buf += writen;
        if (!size) return LastError = SERIAL_ERR_OK;  // Done
...
int CRSSerial::SendChar(int c) {
  ...
  if (ComHandler >= 0) {
    ...
    if (FD_ISSET(ComHandler, &fds)) {
      ...
      if (err == 0) return LastError = SERIAL_ERR_SBFULL;
    } else
...
int CRSSerial::ReceiveChar() {
  ...
  if (ComHandler >= 0) {
    ...
    if (FD_ISSET(ComHandler, &fds)) {
      ...
      if (err < 0) return LastError = SERIAL_ERR_GENERAL;
      if (err > 0) return ch;
    }
  } else {
    return LastError = SERIAL_ERR_NOTOPENED;
...
bool CRSSerial::IsValidAddress(const char* address) { return true; }

int CRSSerial::GetControlLinesStatus() { return 0; }

#endif

int CRSSerial::SetComParams(unsigned long baudRate, unsigned long dataBits,
                            int parity, unsigned long stopBits) {
...
CSerialType CRSSerial::GetConnectorType() { return serialTypeRS; }
...
int CRSSerial::IoCtl(unsigned long code, unsigned long lParam, void* bParam) {
  ...
  switch (code) {
    ...
    case SERIAL_IOCTL_GET_MODEM_MASK:
      ...
      if (rtn > 0) {
        if (bParam) *(unsigned long*)bParam = lParam ? rtn & lParam : rtn;
        rtn = 0;
      }
      break;
...
int CRSSerial::CheckModemReadyMask() {
  if (ModemReadyMask) {  // Just for optimization
    ...
    if (err >= 0)
      LastError = ((ModemReadyMask == (ModemReadyMask & (unsigned long)err))
                       ? SERIAL_ERR_OK
                       : SERIAL_ERR_NOTOPENED);
    // else
