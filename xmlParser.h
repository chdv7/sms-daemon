...
public:
     ...
     * the XML document is corrupted, the "parseString" method will initialize
     * the "pResults" variable with some information that can be used to trace
     * the error. If you still want to parse the file, you can use the
     * APPROXIMATE_PARSING option as explained inside the note at the beginning
     * of the "xmlParser.cpp" file.
     *
     ...
     * XMLNode tree. The "opposite" of this function is the function
     * "writeToFile" that re-creates an XML file from an XMLNode tree. If the
     * XML document is corrupted, the "parseFile" method will initialize the
     ...
     * and errors that occurred during parsing of the XML file. This function
     * also tries to guess char Encoding (UTF-8, ASCII or SHIT-JIS) based on the
     * first 200 bytes of the file. Since each application has its own way to
     * report and deal with errors, you should rather use the "parseFile"
     * function to parse XML files and program yourself thereafter an "error
     * reporting" tailored for your needs (instead of using the very crude
     * "error reporting" mechanism included inside the "openFileHelper"
     * function).
     *
     ...
     * tailored to your needs. If you still want to parse the file, you can use
     * the APPROXIMATE_PARSING option as explained inside the note at the
    ...
    static XMLCSTR getError(XMLError error); ///< this gives you a user-friendly explanation of the
                                             ///< parsing error
     ...
     * and carriage returns. if pnSize is given it returns the size in character
     * of the string. */
    ...
    /**< If nFormat==0, no formatting is required otherwise this returns an user
     * friendly XML string from a given element with appropriate white spaces
     * and carriage returns. If the global parameter
    ...
    XMLNode getChildNode(XMLCSTR name, int i) const; ///< return ith child node with specific name (return an empty
                                                     ///< node if failing). If i==-1, this returns the last XMLNode
                                                     ///< with the given name.
    XMLNode getChildNode(XMLCSTR name, int* i = NULL) const; ///< return next child node with specific name (return an
    ...
    XMLCSTR getAttribute(XMLCSTR name, int i) const;          ///< return ith attribute content with
    ...
    XMLNodeContents enumContents(XMLElementPosition i) const; ///< enumerate all the different contents
                                                              ///< (attribute,child,text, clear) of the current XMLNode.
                                                              ///< The order is reflecting the order of the original
                                                              ///< file/string. NOTE: 0 <= i < nElement();
     ...
     * update) a XMLNode structure. Start by creating your top node with the
     ...
     * inserts at the end. The value (pos=0) insert at the beginning (Insertion
     * at the beginning is slower than at the end). <br>
     *
    ...
    static XMLNode createXMLTopNode(
        XMLCSTR lpszName,
        char isDeclaration = FALSE); ///< Create the top node of an XMLNode structure
    XMLNode addChild(
        XMLCSTR lpszName, char isDeclaration = FALSE,
        XMLElementPosition pos = -1); ///< Add a new child node
    XMLNode addChild(
        XMLNode nodeToAdd,
        XMLElementPosition pos = -1); ///< If the "nodeToAdd" has some parents, it will
    ...
    XMLClear* addClear(
        XMLCSTR lpszValue, XMLCSTR lpszOpen = NULL, XMLCSTR lpszClose = NULL, XMLElementPosition pos = -1);
    /**< Add a new clear tag
    ...
    XMLAttribute* updateAttribute(
        ...
        int i = 0); ///< if the attribute to update is missing, a new one will
                    ///< be added
    XMLAttribute* updateAttribute(
     ...
     * subtree that has been deleted CANNOT be used anymore!!. Unexpected
     * results will occur if you continue using them. */
    void deleteAttribute(int i = 0);                 ///< Delete the ith attribute of the current XMLNode
    void deleteAttribute(XMLCSTR lpszName);          ///< Delete the attribute with the given name (the
                                                     ///< "strcmp" function is used to find the right
                                                     ///< attribute)
    void deleteAttribute(XMLAttribute* anAttribute); ///< Delete the attribute with the name
    ...
    void deleteText(XMLCSTR lpszValue);              ///< Delete the text content "lpszValue" inside the
    ...
    void deleteClear(XMLCSTR lpszValue);             ///< Delete the clear tag "lpszValue" inside the
                                                     ///< current XMLNode (direct "pointer-to-pointer"
                                                     ///< comparison is used to find the clear tag)
    void deleteClear(XMLClear* p);                   ///< Delete the clear tag "p" inside the current XMLNode (direct
    ...
    XMLClear* updateClear_WOSD(
        XMLSTR lpszNewContent,
        int i = 0);                                             ///< if the clearTag to update is
                                                                ///< missing, a new one will be added
    XMLClear* updateClear_WOSD(XMLClear* newP, XMLClear* oldP); ///< if the clearTag to update is
                                                                ///< missing, a new one will be added
    XMLClear* updateClear_WOSD(
        XMLSTR lpszNewValue,
        XMLCSTR lpszOldValue); ///< if the clearTag to update is missing, a new
     ...
     * text or a XMLClearTag in the middle (at a specified position) of a
     * XMLNode tree already constructed. The value returned by these methods is
     * to be used as last parameter (parameter 'pos') of addChild, addText or
     * addClear.
     * @{ */
    ...
    XMLElementPosition positionOfChildNode(XMLCSTR name, int i = 0) const; ///< return the position of the ith childNode
                                                                           ///< with the specified name if (name==NULL)
                                                                           ///< return the position of the ith childNode
    /** @} */
    ...
    static char setGlobalOptions(
        XMLCharEncoding characterEncoding = XMLNode::char_encoding_UTF8, char guessWideCharChars = 1,
     ...
     * is compiled in WideChar mode, then the XMLNode::parseFile and
     ...
     *
     * @param guessWideCharChars If "guessWideCharChars"=1 and if this library
     * is compiled in ASCII/UTF8/char* mode, then the XMLNode::parseFile and
     ...
     * XML-compliant, you should set dropWhiteSpace=0. A note of caution: if you
     * set "dropWhiteSpace=0", the parser will be slower and your code will be
     * more complex.
     *
     * @param removeCommentsInMiddleOfText To explain this parameter, let's
     * consider this code: \code XMLNode x=XMLNode::parseString("<a>foo<!--
     * hello -->bar<!DOCTYPE world >chu</a>","a"); \endcode If
     ...
     * the appropriate value of the global parameter "characterEncoding"
     * described in the XMLNode::setGlobalOptions. The guess is based on the
     * content of a buffer of length "bufLen" bytes that contains the first
     * bytes (minimum 25 bytes; 200 bytes is a good value) of the file to be
     * parsed. The XMLNode::openFileHelper function is using this function to
     * automatically compute the value of the "characterEncoding" global
     * parameter. There are several heuristics used to do the guess. One of the
     * heuristic is based on the "encoding" attribute. The original XML
...
public:
     ...
     * of "destination-buffer-overflow". It converts the string
...
public:
     ...
     * encoding of "inByteLen" bytes from "inByteBuf". If "formatted" parameter
     * is true, then there will be a carriage-return every 72 chars. The string
     * will be free'd when the XMLParserBase64Tool object is deleted. All
     * returned strings are sharing the same memory space. */
    XMLSTR encode(
    ...
    unsigned char* decode(
        ...
        XMLError* xe = NULL); ///< returns a pointer to an internal buffer containing the
                              ///< binary data decoded from "inString"

    /**
     * decodes data from "inString" to "outByteBuf". You need to provide the
     * size (in byte) of "outByteBuf" in "inMaxByteOutBuflen". If "outByteBuf"
     * is not large enough or if data is malformed, then "FALSE" will be
